    ...
    <form method="POST" action="{{ route('cliente.update', ['cliente' => $cliente->id]) }}">
      @method('put')
        ...
        <div class="mb-3">
          ...
          name="apellido" placeholder="Apellido."value="{{ $cliente->apellido }}">
        </div>
        <div class="mb-3">
          <label for="telefono" class="form-label">Telefono</label>
          <input type="text" class="form-control" id="telefono" aria-describedby="nameHelp"
          name="telefono" placeholder="Telefono." value="{{ $cliente->telefono }}">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          name="direccion" placeholder="Direccion." value="{{ $cliente->direccion }}">
        </div>
        
        <div class="mt-3">
        <button type="submit" class="btn btn-primary">save</button>
        <a href="{{ route('cliente.index') }}" class="btn btn-warning">Cancel</a>
        </div>
      </form>
